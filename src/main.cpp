...
unsigned int nWorkTargetSpacing = 120;                  // 60 sec block spacing for PoW
...
const string strMessageMagic = "EqualCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // EqualCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // EqualCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
static int FINAL_POW_HEIGHT = 2000000;
static int FINAL_POW_COUNT = 262800;
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, const CBlockIndex* pindex)
{
	...
	if(nPoWHeight > LAST_POW_ONLY_BLOCK)
		return 0;
...
static const int64_t ifac1 = 999;		
static const int64_t ifac2 = 99;	
static const int64_t ifac3 = 9;
...
int64_t GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
	...
	int nPoWHeight = GetPowHeight(pindex);
	if(pindex->nHeight < 3 || nPoWHeight > LAST_POW_ONLY_BLOCK)
		return 0;

	uint256 hash = pindex->GetBlockHash();
	std::string cseed_str = hash.ToString().substr(14,7);
	const char* cseed = cseed_str.c_str();
	long seed = hex2long(cseed);
	int random = generateMTRandom(seed, 216000);

	// printf(">> cseed = %s, random = %d\n", cseed, random);

	if(random > 175880 && random < 175901)		
	{
		return ifac1;
	}
	else if(random > 45700 && random < 45851)	
	{
		return ifac2;
	}
	else if (random > 119100 && random < 120001)
	{
		return ifac3;
	}
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, const CBlockIndex* pindex)
{
	...
	{
		nSubsidy = 12 * nRewardCoinYear * nCoinAge / 365;
	}
	else if(nPoSHeight < 3 * YEARLY_POS_BLOCK_COUNT)	
	{
		nSubsidy = 6 * nRewardCoinYear * nCoinAge / 365;
	}
...
// EqualCoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // EqualCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // EqualCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // EqualCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // EqualCoin: track money supply and mint amount info
...
// EqualCoin: total coin age spent in transaction, in the unit of coin-days.
...
// EqualCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // EqualCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // EqualCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // EqualCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // EqualCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
	...
	int powHeight = GetPowHeight(pindexPrev);
	int nHeight = pindexPrev->nHeight + 1;
    if (IsProofOfWork() && powHeight > LAST_POW_ONLY_BLOCK)
    ...
    // EqualCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // EqualCoin: check proof-of-stake
    ...
    // EqualCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // EqualCoin: getblocks may not obtain the ancestor block rejected
    ...
    // EqualCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EqualCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "EqualCoin: the coin for equality!!!";
        CTransaction txNew;
        txNew.nTime = 1476685282;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1476685282;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1669876;
        if(fTestNet)
        {
            block.nNonce = 0;
        }
		...
		*/
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("e9bb786a004d3f663458ffbc3a12acc37d0ebd836452601e68576a82813c9916"));
        // block.print();
        ...
        // EqualCoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0xc3, 0xda, 0xf1, 0xfb };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // EqualCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // EqualCoin: tell downloading node about the latest block if it's
